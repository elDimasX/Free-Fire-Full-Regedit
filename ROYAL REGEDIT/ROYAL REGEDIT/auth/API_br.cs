...
namespace Auth.GG_Winform_Example
{
        ...
        public static void Initialize(string name, string aid, string secret, string version)
        {
            ...
            using (WebClient wc = new WebClient())
            {
                ...
                try
                {
                    ...
                    {
                        MessageBox.Show("Possível atividade maliciosa detectada!", OnProgramStart.Name);
                        Process.GetCurrentProcess().Kill();
                    }
                    ...
                    {
                        MessageBox.Show("Erro de segurança acionado!");
                        Process.GetCurrentProcess().Kill();
                    }
                    switch (response[2])
                    {
                        case "success":
                            ...
                            {
                                MessageBox.Show("O aplicativo está no modo de desenvolvedor, ignorando a integridade e a verificação de atualização!");
                                ...
                                MessageBox.Show("O hash de seu aplicativo foi salvo em Integrity.txt, consulte-o quando seu aplicativo estiver pronto para lançamento!");
                            }
                            else
                            {
                                if (response[12] == "Enabled")
                                {
                                    ...
                                    {
                                        MessageBox.Show($"O arquivo foi adulterado, não foi possível verificar a integridade!");
                                        Process.GetCurrentProcess().Kill();
                                    }
                                }
                            ...
                            {
                                MessageBox.Show("parece que este aplicativo está desativado, tente novamente mais tarde!");
                                Process.GetCurrentProcess().Kill();
                            }
                            break;
                        case "binderror":
    ...
    internal class API
    {
        public static void Log(string username, string action)
        {
            ...
            {
                MessageBox.Show("Inicialize seu aplicativo primeiro!", OnProgramStart.Name);
                Process.GetCurrentProcess().Kill();
            }
            if (string.IsNullOrWhiteSpace(action))
            {
                MessageBox.Show("Informações de registro ausentes!", ApplicationSettings.Name);
                Process.GetCurrentProcess().Kill();
            }
            string[] response = new string[] { };
            using (WebClient wc = new WebClient())
        ...
        public static bool AIOLogin(string AIO)
        {
            ...
            using (WebClient wc = new WebClient())
            {
                try
                {
                    ...
                    if (response[0] != Constants.Token)
                    {
                        MessageBox.Show("Erro de segurança acionado!", OnProgramStart.Name);
                        Process.GetCurrentProcess().Kill();
                    }
                    if (Security.MaliciousCheck(response[1]))
                    {
                        MessageBox.Show("Posible actividad maliciosa detectada!", OnProgramStart.Name);
                        Process.GetCurrentProcess().Kill();
                    }
                    if (Constants.Breached)
                    {
                        MessageBox.Show("PPossível atividade maliciosa detectada!", OnProgramStart.Name);
                        Process.GetCurrentProcess().Kill();
                    }
                    switch (response[2])
                    {
                        ...
                        case "invalid_details":
                            ...
                            return false;
                        case "time_expired":
                            MessageBox.Show("Seu  plan expirou!", ApplicationSettings.Name);
                            Security.End();
                            return false;
                        case "hwid_updated":
                            MessageBox.Show("Uma nova máquina foi ligada, reabra o aplicativo!", ApplicationSettings.Name);
                            Security.End();
                            return false;
                        case "invalid_hwid":
                            MessageBox.Show("Este usuário está vinculado a outro computador, entre em contato com o suporte!", ApplicationSettings.Name);
                            Security.End();
                            return false;
                    }
                }
        ...
        public static bool AIORegister(string AIO)
        {
            ...
            if (string.IsNullOrWhiteSpace(AIO))
            {
                MessageBox.Show("Informações de registro inválidas!", ApplicationSettings.Name);
                Process.GetCurrentProcess().Kill();
            }
            string[] response = new string[] { };
            using (WebClient wc = new WebClient())
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        case "success":
                            ...
                            return true;
                        case "invalid_details":
                            MessageBox.Show("Desculpe, seu nome de usuário / senha não confere!", ApplicationSettings.Name);
                            Security.End();
                            Environment.Exit(0);
                            return false;
                        case "time_expired":
                            MessageBox.Show("Seu plano expirou!", ApplicationSettings.Name);
                            Security.End();
                            Environment.Exit(0);
                            return false;
                        case "hwid_updated":
                            MessageBox.Show("Uma nova máquina foi ligada, reinicie o aplicativo!", ApplicationSettings.Name);
                            Security.End();
                            Environment.Exit(0);
                            return false;
                        case "invalid_hwid":
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        ...
                        case "invalid_license":
                            MessageBox.Show("Licença não existe!", ApplicationSettings.Name);
                            Security.End();
                            Environment.Exit(0);
                            return false;
                        case "email_used":
                            MessageBox.Show("Email já foi usado!", ApplicationSettings.Name);
                            Security.End();
                            Environment.Exit(0);
                            return false;
                        case "invalid_username":
                            MessageBox.Show("Você inseriu um nome de usuário inválido / usado!", ApplicationSettings.Name);
                            Security.End();
                            Environment.Exit(0);
                            return false;
                    }
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        case "success":
                            ...
                            return true;
                        case "invalid_token":
                            MessageBox.Show("O token não existe!", ApplicationSettings.Name);
                            Security.End();
                            return false;
                        case "invalid_details":
                            MessageBox.Show("Seus dados de usuário são inválidos!", ApplicationSettings.Name);
                            Security.End();
                            return false;
                    }
                }
    ...
    internal class Security
    {
        ...
        public static void Start()
        {
            ...
            {
                MessageBox.Show("Uma sessão já foi iniciada, termine a anterior!", OnProgramStart.Name);
                Process.GetCurrentProcess().Kill();
            }
            else
            {
                using (StreamReader sr = new StreamReader($@"{drive}Windows\System32\drivers\etc\hosts"))
                {
                    ...
                    {
                        Constants.Breached = true;
                        MessageBox.Show("O redirecionamento de DNS foi detectado!", OnProgramStart.Name);
                        Process.GetCurrentProcess().Kill();
                    }
                }
        ...
        public static void End()
        {
            if (!Constants.Started)
            {
                MessageBox.Show("Nenhuma sessão foi iniciada. Fechando por motivos de segurança!", OnProgramStart.Name);
                Process.GetCurrentProcess().Kill();
            }
            else
            {
